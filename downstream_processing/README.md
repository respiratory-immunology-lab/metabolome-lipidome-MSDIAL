...
Like the continuous function, the `metab_limma_categorical()` function takes a `SummarizedExperiment` object as input. This function however takes a categorical variable: `metadata_var` as its secondary input; this must be a string value that matches the column name of a categorical metadata value in your `SummarizedExperiment` object. You can place conditions on your metadata using the `metadata_condition` variable, which can be useful if you want to restrict your analysis to a specific subset of data, e.g. a certain time-frame.

##### Arguments

- `metab_SE`: a metabolomics/lipidomics `SummarizedExperiment` object produced by the `pmp_preprocess()` function.
- `metadata_var`: the test variable for limma analysis; must be a string value matching the column name of a categorical metadata value in your `SummarizedExperiment` object.
- `metadata_condition`: a conditional statement relating to a metadata value in your `SummarizedExperiment` object that will be used to subset the data.
- `model_matrix`: a model matrix generated by the `limma` `model.matrix()` function.
- `contrast_matrix`: a model constrast matrix generated by the `limma` `makeConstrasts()` function.
- `adjust_method`: (default: 'BH') the method for adjusting the p-values for multiple testing. Options, in increasing conservatism, include `'none'`, `'BH'`, `'BY'`, and `'holm'`. See [`p.adjust`](https://www.rdocumentation.org/link/p.adjust?package=limma&version=3.28.14) for the complete list of options. A `NULL` value will result in the default adjustment method, which is `'BH'`.
- `rownames`: a vector of feature names with the same length as the number of features (defaults to the `shortname` metadata column if not provided).
- `adj_pval_threshold`: (default: 0.05) the maximum adjusted p-value (q-value) that is considered statistically significant.
- `logFC_threshold`: (default: 1) the minimum log2 fold-change that is considered scientifically meaningful.
- `legend_metadata_string`: a custom heading the legend of the volcano plot that shows up before the description of the contrast being made.
- `volc_plot_title`: a custom title for the volcano plot.
- `volc_plot_subtitle`: a custom subtitle for the volcano plot.
- `volc_plot_xlab`: a custom label for the x-axis of the volcano plot.
- `volc_plot_ylab`: a custom label for the y-axis of the volcano plot.
